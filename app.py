...
app.config['MAIL_USERNAME'] = 'cctankinshasa@gmail.com'
app.config['MAIL_PASSWORD'] = 'dsyu oasp army bhxn'
app.config['MAIL_DEFAULT_SENDER'] = 'cctankinshasa@gmail.com'
...
def envoyer_email(destinataire, code):
    ...
    try:
        with smtplib.SMTP_SSL('smtp.gmail.com', 465) as serveur:
            serveur.login("cctankinshasa@gmail.com", "dsyu oasp army bhxn")
            serveur.sendmail("cctankinshasa@gmail.com", destinataire, texte)
...
def confirmation_connexion():
    if request.method == 'POST':
        ...
        if code_saisi == code_envoye:
            ...
            return redirect(url_for('inscription_candidat'))
        else:
...
def inscription_candidat():
    ...
    professions = cur.fetchall()
    conn.close()
    return render_template('inscription_candidat.html', offres=offres, professions=professions, email_connecte=email_connecte)
...
@app.route('/valider_candidature/<int:id>', methods=['POST'])
def valider_candidature(id):
    conn = connect_db()
    cur = conn.cursor(dictionary=True)

    # 1. Vérifier si le candidat existe
    cur.execute("SELECT email, nom, prenom FROM candidats WHERE id = %s", (id,))
    candidat = cur.fetchone()
    ...
    date_interview = request.form.get('date_interview')
    heure_interview = request.form.get('heure_interview')

    # 3. Mettre à jour la base
    cur.execute("""
        UPDATE candidats
        SET date_interview = %s, heure_interview = %s
        WHERE id = %s
    """, (date_interview, heure_interview, id))
    conn.commit()
    ...
    try:
        msg = Message(
            "Confirmation provisoire de votre candidature",
            recipients=[email]
        )
        msg.body = f"""
Bonjour {prenom} {nom},

La direction des Ressources Humaines de {nom_etablissement} vous convoque pour un entretien
le {date_interview} à {heure_interview}.

Merci de respecter toutes les instructions exigées pour accéder au sein de notre bâtiment.
...
"""
        ...
        flash("Candidature validée mais erreur lors de l’envoi de l’email : " + str(e), "warning")
    else:
        flash("Candidature validée et email envoyé avec succès", "success")
